module("Ember.Validators.getValidator", {
  setup: function() {
    Ember.Validators.ExistentValidator = Ember.Validator.extend();
  },
  teardown: function() {
    delete Ember.Validators.ExistentValidator;
  }
});
...
test("should instantiate validator from options when set", function() {
  var validatorClass = Ember.Validator.extend();
...
test("should find the validator whose name matches", function() {
  ...
  equal(validator.getPath('options.opt'), 'foo', "should set validator options");
});

test("should set 'option' validator property when options is not a hash", function() {
  var validator = Ember.Validators.getValidator('existent', 12);

  equal(validator.get('options'), 12, "should set 'option' property to the argument value");
});